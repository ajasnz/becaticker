...
- Chain 1: 5x1 horizontal panels (320x64) for text display
- Chain 2: 2x2 panels (128x128) for analog clock display

Physical Layout:
[Panel1][Panel2][Panel3][Panel4][Panel5]  [Clock-TL][Clock-TR]
                                          [Clock-BL][Clock-BR]

Chain 2 is positioned to the right of Chain 1, with the bottom row
aligned with Chain 1 and extending upward.
    ...
    def __init__(self, config_file: str = "config.json"):
        ...
        self.default_config = {
            ...
            "matrix_options": {
                "rows": 64,
                "cols": 64,
                "chain_length": 5,  # Chain 1: 5 panels in a row
                "parallel": 2,  # Chain 2: 2x2 arrangement (4 panels total)
                "brightness": 40,
                "gpio_mapping": "regular",
                "gpio_slowdown": 4,
                "hardware_mapping": "regular",
                "pixel_mapper_config": "U-mapper",  # For 2x2 panel arrangement
            },
            "display_settings": {
...
class AnalogClock:
    """Handles analog clock display on Chain 2 (2x2 panels)."""

    def __init__(self, matrix: RGBMatrix, config: Config, row_offset: int = 64):
        self.matrix = matrix
        self.config = config
        self.canvas = None  # Will be set by main loop
        self.row_offset = row_offset  # Chain 2 starts at row 64 (below chain 1)
...
class BecaTicker:
    ...
    def __init__(self):
        ...
        # Initialize dual chain matrix: Chain 1 (5x1 text panels) + Chain 2 (2x2 clock panels)
        self.matrix = self._create_matrix()

        # Initialize displays with different row offsets for parallel chains
        # Chain 1: Text display (5x1 horizontal panels, 320x64)
        self.text_display = TextDisplay(
            self.matrix, self.config, self.calendar_manager, row_offset=0
        )

        # Chain 2: Analog clock display (2x2 panels, 128x128)
    ...
    def _create_matrix(self) -> RGBMatrix:
        """Create and configure dual-chain RGB matrix setup."""
        ...
        options.chain_length = chain_config.get("chain_length", 5)  # Chain 1: 5 panels
        options.parallel = chain_config.get("parallel", 2)  # Chain 2: 2x2 = 4 panels
        ...
        # Additional anti-flickering options for dual chain setup
        ...
        logger.info(
            f"Creating dual-chain matrix: Chain1({options.chain_length}x1 = {options.chain_length * options.cols}x{options.rows}), "
            f"Chain2(2x2 = {2 * options.cols}x{2 * options.rows})"
        )
    ...
    def _display_loop(self) -> None:
        """Main display update loop for both parallel chains."""
        logger.info("Display update loop started - dual chain mode")
        ...
        while self.running:
            try:
                ...
                # Update both displays (they draw to different regions of the same canvas)
                # Chain 1: Text display (rows 0-63, cols 0-319)
                self.text_display.update_display()

                # Chain 2: Analog clock display (rows 64-191, cols 0-127)
                self.analog_clock.update_display()

                # Swap the canvas buffers once for both chains
