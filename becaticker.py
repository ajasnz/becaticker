...
class ClockDisplay:
    ...
    def _map_2x2_coordinates(self, logical_x: int, logical_y: int) -> tuple:
        ...
        if panel_y == 0:  # Top row
            if panel_x == 0:  # Top-left -> currently shows bottom-right, try bottom-left panel
    ...
    def _draw_analog_clock(self, now: datetime, colors: dict) -> None:
        ...
        self._draw_circle(
            self.center_x, self.center_y, self.clock_radius, colors["face"], fill=False
        )

        # Draw hour markers
        for hour in range(12):
            angle = (
                (hour * 30 - 90) * 3.14159 / 180
            )  # Convert to radians, -90 for 12 at top

            # Outer and inner radius for hour markers
            outer_x = int(self.center_x + (self.clock_radius - 5) * math.cos(angle))
            outer_y = int(self.center_y + (self.clock_radius - 5) * math.sin(angle))
            inner_x = int(self.center_x + (self.clock_radius - 15) * math.cos(angle))
            inner_y = int(self.center_y + (self.clock_radius - 15) * math.sin(angle))

            self._draw_line(inner_x, inner_y, outer_x, outer_y, colors["ticks"])
            ...
            num_x = int(self.center_x + (self.clock_radius - 25) * math.cos(angle))
            num_y = int(self.center_y + (self.clock_radius - 25) * math.sin(angle))
        ...
        hour_angle = ((now.hour % 12) * 30 + now.minute * 0.5 - 90) * 3.14159 / 180
        minute_angle = (now.minute * 6 - 90) * 3.14159 / 180
        second_angle = (now.second * 6 - 90) * 3.14159 / 180

        # Draw hour hand (shorter, thicker)
        hour_length = self.clock_radius * 0.5
        hour_end_x = int(self.center_x + hour_length * math.cos(hour_angle))
        hour_end_y = int(self.center_y + hour_length * math.sin(hour_angle))
        ...
        minute_end_x = int(self.center_x + minute_length * math.cos(minute_angle))
        minute_end_y = int(self.center_y + minute_length * math.sin(minute_angle))
        self._draw_line(
            self.center_x,
            self.center_y,
            minute_end_x,
        ...
        second_end_x = int(self.center_x + second_length * math.cos(second_angle))
        second_end_y = int(self.center_y + second_length * math.sin(second_angle))
        self._draw_line(
            self.center_x,
            self.center_y,
            second_end_x,
        ...
        # Draw center dot
        self._draw_circle(
            self.center_x, self.center_y, 2, colors["hour_hand"], fill=True
        )

    def _draw_digital_time(self, now: datetime, colors: dict) -> None:
